...
结点C没有任何outcoming links，但有一条指向自身的边，这种情况就是spider trap。对于这种情况，使用公式迭代计算的时，向量`v`中，C结点对应的权重值会
...
还有trap以外的结点，概率还会通过这些结点来继续往外扩散，但进入trap的这部分概率会被这几个trap中的结点完全占有。
因为trap只有入口，没有出口，所以迭代的过程中还会继续有概率进入这个trap，最终概率会被这几个trap中的结点全部占尽。
...
有兴趣的朋友可以使用R跑一下上面代码，你会发现第3个值，也就是结点C对应的值会越来大，最后几乎就是1，其它结点对应的值
